...
// building the ALIGNER circuitry
	...
	for(auto& enode : *enode_dag) {
		if(enode->is_if_else_cmerge || enode->is_data_loop_cmerge) {
			...
			for(auto& e:*enode_dag) {
				// replaced the above code with directly accessing the stored tagger/untagger to avoid mistakes in the presence of multiple taggers/untaggers in the circuit
...
void CircuitGenerator::insert_tagger_untagger_if_else_node(ENode* if_else_master_cmerge) {
	...
	// changed the above code to put the untagger in the BB that is the succ of the if_else_master_cmerge
...
// the goal of this function is to identify components inside a loop as tagged by setting the tagged flag of the enode to true
