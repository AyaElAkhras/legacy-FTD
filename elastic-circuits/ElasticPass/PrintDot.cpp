...
// 25/06/2025: added a field for marking the units that need to be fed 1-bit 
void labelBooleanOperations(std::vector<ENode*>* enode_dag) {
	for(auto& enode : *enode_dag) {
		if(enode->type == Inst_)
			if(enode->Instr->getOpcode() == Instruction::ICmp || enode->Instr->getOpcode() == Instruction::FCmp)
				enode->isBooleanOper = true;

		if(enode->is_merge_init)
			enode->isBooleanOper = true;

		for (auto& pred : *enode->CntrlPreds) {
			if(pred->isBooleanOper)
				enode->isBooleanOper = true;

			if(pred->type == Inst_)
				if(pred->Instr->getOpcode() == Instruction::ICmp || pred->Instr->getOpcode() == Instruction::FCmp)
					enode->isBooleanOper = true;
		}
	}

	// Do a second round: ideally this should be done recursively
	for(auto& enode : *enode_dag) {
		for (auto& pred : *enode->CntrlPreds) {
			if(pred->isBooleanOper)
				enode->isBooleanOper = true;
	...
	for(auto& enode : *enode_dag) {
		for (auto& succ : *enode->CntrlSuccs) {
			if(succ->type == Inst_)
				if(succ->Instr->getOpcode() == Instruction::ICmp || succ->Instr->getOpcode() == Instruction::FCmp)
					continue;
			if(succ->isBooleanOper)
				enode->isBooleanOper = true;
		}
	}
...
void aya_printDotDFG(std::vector<ENode*>* enode_dag, std::vector<BBNode*>* bbnode_dag, std::string name, std::string serial_number, bool fix_mc_st_interfaces_flag, const std::string& tag_info_path) {
    ...
    */
    labelBooleanOperations(enode_dag);

    aya_printDotNodes(enode_dag, serial_number, bbnode_dag, fix_mc_st_interfaces_flag, tag_info_path);
	
	aya_printotherNodes(enode_dag, serial_number, fix_mc_st_interfaces_flag, tag_info_path);  // print the nodes that are not inside any BB!
	dotfile <<"\n";
...
std::string aya_printDataflowEdges(std::vector<ENode*>* enode_dag, std::vector<BBNode*>* bbnode_dag, bool fix_mc_st_interfaces_flag) {
	...
	*/ 
	...
	for (auto& enode : *enode_dag) {
		...
		// Aya: 11/1/2025: Skip the new Split and Concat nodes to handle them separately because they need special treatment since the succs of the Split for instance can be the same so will always print the index of the first one
		if(enode->type == Split || enode->type == Split_tag || enode->type == Concat || enode->type == Concat_tag)
			continue;
...
////////////////////////////////////////////////// END OF THE LSQ CONTROL NETWORK STUFF!!!
	...
	// Aya: 11/1/2025: Handling the new Split and Concat nodes separately. This is added just to test the correctness of the backend modules, but the Lean framework will be the one adding those units and printing their Dot
	for (auto& enode : *enode_dag) {
		// The goal is to print the connections by looking at the enodes and their succs

		if(enode->type == Split || enode->type == Split_tag) {
			assert(enode->CntrlPreds->size() == 1);
			assert(enode->CntrlSuccs->size() == 2);
			str += printEdge(enode, enode->CntrlSuccs->at(0)); // print that they are connected
			str += " [";
			str += printColor(enode, enode->CntrlSuccs->at(0));
			str += ", from = \"out" + to_string(1) + "\"";
			str += ", to = \"in" + to_string(1) + "\"];\n";   // works mainly for my precise test case of Split directly feeding the 2 inputs of Concat
		

			str += printEdge(enode, enode->CntrlSuccs->at(1)); // print that they are connected
			str += " [";
			str += printColor(enode, enode->CntrlSuccs->at(1));
			str += ", from = \"out" + to_string(2) + "\"";
			str += ", to = \"in" + to_string(2) + "\"];\n";    // works mainly for my precise test case of Split directly feeding the 2 inputs of Concat
		
		} else if(enode->type == Concat || enode->type == Concat_tag) {
			...
			str += printColor(enode, enode->CntrlSuccs->at(0));
			str += ", from = \"out" + to_string(1) + "\"";
			str += ", to = \"in" + to_string(1) + "\"];\n";    // works mainly for my precise test case of Concat directly feeding return
...
int retrieve_number_of_tags_from_input(const std::string& tag_info_path) {
...
void aya_printotherNodes(std::vector<ENode*>* enode_dag, std::string serial_number, bool fix_mc_st_interfaces_flag, const std::string& tag_info_path) {
// Target is to loop over basic blocks and print the operations existing in each basic block
	string dotline = "";

	int tags_num = retrieve_number_of_tags_from_input(tag_info_path);
	int tag_bit_size = floor(log2(tags_num)) + 1;  //add 1 because it is not an index
		...
		for (auto& enode : *enode_dag) {
			...
			if(!skipNodePrint(enode) && enode->BB == nullptr){
				...
				if (OptimizeBitwidth::isEnabled()) {
					...
					dotline += getNodeDotInputs(enode, fix_mc_st_interfaces_flag, tag_bit_size);
					dotline += getNodeDotOutputs(enode, fix_mc_st_interfaces_flag, tag_bit_size);
				}
				/////////////////////
				...
				// AYA: 27/04/2024: added the following to add the tag fields to buffers in case we use naive buffering
				if(enode->type == Bufferi_ || enode->type == Buffera_) {
					dotline += ", tagged=";
					if(enode->is_tagged)
						dotline += "true";
					else
...
void aya_printDotNodes(std::vector<ENode*>* enode_dag, std::string serial_number, std::vector<BBNode*>* bbnode_dag, bool fix_mc_st_interfaces_flag, const std::string& tag_info_path) {
// Target is to loop over basic blocks and print the operations existing in each basic block
	string dotline = "";
	int count = 0;
...
int getInPortSize(ENode* enode, int index) {
	...
	if(enode->isBooleanOper && enode->type != Inst_ && enode->type != Cst_)
		return COND_SIZE;

    if (enode->type == Argument_ || enode->type == Aligner_Mux || enode->type == Aligner_Branch) { 
        return DATA_SIZE;
...
int getOutPortSize(ENode* enode, int index) {
	if(enode->isBooleanOper && enode->type != Inst_)
		return COND_SIZE;
    ...
    } else if (enode->type == Phi_ || enode->type == Phi_n || enode->type == Loop_Phi_n || enode->type == Cst_) {  
        return DATA_SIZE;
    } else {
...
std::string getNodeDotInputs(ENode* enode, bool fix_mc_st_interfaces_flag, int tag_bit_size) {
    string name = "";
    ...
    switch (enode->type) {
        ...
        case End_:
            ...
            break;
        case Split:
        	name += ", in = \"in1:" + to_string(DATA_SIZE) + "\"";  // TODO: should be the sum of the bitwidths of the 2 inputs (assuming that we properly propagate it for the inputs)
        	break;
    	case Split_tag:
        	name += ", in = \"in1:" + to_string(tag_bit_size + DATA_SIZE) + "\"";  // TODO: should be the sum of the bitwidths of the 2 inputs (assuming that we properly propagate it for the inputs)
        	break;
    	case Concat:
    		name += ", in = \"in1:" + to_string(DATA_SIZE / 2) + " in2:" + to_string(DATA_SIZE / 2) + "\""; // TODO: should propagate whatever comes at the two inputs
    		break;
		case Concat_tag:
    		name += ", in = \"in1:" + to_string(DATA_SIZE) + " in2:" + to_string(tag_bit_size) + "\""; // TODO: should propagate whatever comes at one inputs
    		break;
        default:
            break;
    }
...
std::string getNodeDotOutputs(ENode* enode, bool fix_mc_st_interfaces_flag, int tag_bit_size) {
    string name = "";
    ...
    switch (enode->type) {
        ...
        case End_:
            ...
            break;
        case Split:
        	name += ", out = \"out1:" + to_string(DATA_SIZE / 2) + " out2:" + to_string(DATA_SIZE / 2) + "\""; // TODO: should be the sum of the bitwidths of the 2 inputs (assuming that we properly propagate it for the inputs)
        	break;
    	case Split_tag:
        	name += ", out = \"out1:" + to_string(DATA_SIZE) + " out2:" + to_string(tag_bit_size) + "\"";  // TODO: should be the sum of the bitwidths of the 2 inputs (assuming that we properly propagate it for the inputs)
        	break;
    	case Concat:
    		name += ", out = \"out1:" + to_string(DATA_SIZE) + "\"";  // TODO: should propagate whatever comes at the two inputs
    		break;
		case Concat_tag:
    		name += ", out = \"out1:" + to_string(tag_bit_size + DATA_SIZE) + "\""; // TODO: should propagate whatever comes at one inputs
    		break;
        default:
            break;
    }
        ...
        if (!skipNodePrint(enode)) {
            ...
            if (full) {
                ...
                if (OptimizeBitwidth::isEnabled()) {
                    ...
                    dotline += getNodeDotInputs(enode, false, -1);
                    dotline += getNodeDotOutputs(enode, false, -1);
                }
                dotline += getNodeDotParams(enode, serial_number);
                if (enode->type == MC_ || enode->type == LSQ_)
                    dotline += getNodeDotMemParams(enode);
