                                ...
                                else
                                {
                                    ...
                                    // AYA: 05/12/2023: changed the way of calculating the number of bits
                                        ...
                                        if(nodes[i].type.find("Constant") != std::string::npos )
                                            signal += to_string( ( nodes[i].outputs.output[0].bit_size - 1 >= 0 ) ? nodes[i].outputs.output[0].bit_size - 1 : DEFAULT_BITWIDTH-1 );
                                        else
                                            signal += to_string( ( nodes[i].inputs.input[indx].bit_size - 1 >= 0 ) ? nodes[i].inputs.input[indx].bit_size - 1 : DEFAULT_BITWIDTH-1 );
                                    }
                ...
                for ( int in_port_indx = 0; in_port_indx < 1; in_port_indx++) 
                {
                    ...
                    if ( nodes[i].outputs.output[indx].next_nodes_id != COMPONENT_NOT_FOUND )
                    {
                        ...
                        if(nodes[i].type.find("LoopMux") != std::string::npos && indx == 1) {
                            ...
                            {
                                signal_2 = "\"";
                                signal_2 += string_constant ( nodes[nodes[i].outputs.output[indx].next_nodes_id].component_value , nodes[nodes[i].outputs.output[indx].next_nodes_id].outputs.output[0].bit_size );  // Aya: 25: it used to be inputs.input but the input should be 0 bits because the constant is triggered from control
                                signal_2 += "\"";                        
                                netlist << "\t" << signal_1  << " <= " << signal_2 <<SEMICOLOUMN << endl;
                            }
                            else
        ...
        for ( int indx = 0; indx < nodes[i].outputs.size; indx++ ) {
            ...
            if ( nodes[i].outputs.output[indx].next_nodes_id != COMPONENT_NOT_FOUND ) {
                ...
                /*if(nodes[nodes[i].outputs.output[indx].next_nodes_id].type == "ROB") {
                    ...
                    // Aya: 25: commented this out to disable tagged outputs
                    // signal_1 = nodes[nodes[i].outputs.output[indx].next_nodes_id].name;
                    // //signal_1 = (nodes[i].outputs.output[indx].next_nodes_id == COMPONENT_NOT_FOUND ) ? "unconnected" : nodes[nodes[i].outputs.output[indx].next_nodes_id].name;
                    // signal_1 += UNDERSCORE;
                    // signal_1 += "tagIn";
...
string get_generic ( int node_id, bool fix_mc_st_interfaces_flag, bool fix_mc_st_interfaces_flag_yes_extra_ST_output, bool tagged_loop_mux_flag, int max_number_of_nested_taggers )
{
    ...
    // AYA: 11/1/2025
    if(nodes[node_id].type.find("Split") != std::string::npos) {
        generic += to_string(nodes[node_id].outputs.output[0].bit_size);
        generic += COMMA;
        generic += to_string(nodes[node_id].outputs.output[1].bit_size);
    }

    if(nodes[node_id].type.find("Concat") != std::string::npos) {
        generic += to_string(nodes[node_id].inputs.input[0].bit_size);
        generic += COMMA;
        generic += to_string(nodes[node_id].inputs.input[1].bit_size);
    }
    ...
    if ( nodes[node_id].type.find("Constant") != std::string::npos )
    {
        ...
        generic += to_string(nodes[node_id].outputs.size);
        generic += COMMA;
        generic += to_string(nodes[node_id].outputs.output[0].bit_size);    // Aya: 25: it used to be inputs.input but the input should be 0 bits because the constant is triggered from control
        generic += COMMA;
        generic += to_string(nodes[node_id].outputs.output[0].bit_size);
    }    
...
void write_components (bool tagged_loop_mux_flag, bool tagger_is_tagging, int max_number_of_nested_taggers )
{
    ...
    for (int i = 0; i < components_in_netlist; i++) 
    {
        ...
        else if(nodes[i].type == "Split")
        {
            entity += "split";
        }
        else if(nodes[i].type == "Concat")
        {
            entity += "concat";
        }
...
//             netlist << "\t" << "io_queueEmpty" << " => " << input_signal << endl;
    ...
    //          for ( int in_port_indx = 0; in_port_indx < components_type[nodes[i].component_type].in_ports; in_port_indx++) 
                for ( int in_port_indx = 0; in_port_indx < 1; in_port_indx++) 
                {
                    ...
                    if(nodes[i].type.find("Concat") != std::string::npos && indx == 0) {
                        input_port = "dataIn1(0)"; 
                        input_port_2 = "dataIn2(0)";

                    }
                    
                    // AYA: 01/10/2023
                    else if ( (nodes[i].type.find("Aligner_Branch") != std::string::npos && indx == 1) ||
            ...
            // AYA 05/07/2023: Added the following to not print anything in the data of the new third input of the ST (not to be mapped to datainArray)
                    ...
                    */
                    ...
                    if( nodes[i].type.find("ROB") != std::string::npos) {
                        ...
                        } else if(nodes[i].type.find("Concat") != std::string::npos && indx == 0) {
                            input_signal_2 = nodes[i].name;
                            input_signal_2 += UNDERSCORE;
                            input_signal_2 += components_type[nodes[i].component_type].in_ports_name_str[in_port_indx];
                            input_signal_2+= UNDERSCORE;
                            input_signal_2 += to_string( indx + 1);
                        }
                    ...
                    if(indx == 0 && (nodes[i].type == "Synch"))// || nodes[i].type == "Tagger"))  // AYA: 29/04/2023: Added the following condition because we do not have clk and reset in our SYnch component, so we do not want to blindly print a comma in the very beginning 
                       // AYA: 17/09/2023: added the TAGGER to the condition
                        ...
                        if(nodes[i].type.find("ROB") != std::string::npos && indx == 1 || 
                            (tagged_loop_mux_flag && nodes[i].component_operator.find("LoopMux") != std::string::npos && indx == 2) || (nodes[i].type.find("Concat") != std::string::npos && indx == 0) /*||
                            (nodes[i].type == "Un_Tagger")*/){ // AYA: 08/08/2023
                            netlist << COMMA << endl << "\t" << input_port << " => " << input_signal;
                            netlist << COMMA << endl << "\t" << input_port_2 << " => " << input_signal_2;

                        } else if (nodes[i].type.find("Concat") == std::string::npos)  // since we already took care of Concat, print the following only for non-Concat
                            netlist << COMMA << endl << "\t" << input_port << " => " << input_signal;
                    }
                }
            }
...
//                 }
    ...
    //                     else
            ...
            {            
                ...
                for ( int out_port_indx = 0; out_port_indx < components_type[nodes[i].component_type].out_ports; out_port_indx++) 
                {
                    if((nodes[i].type == "Split" && indx == 0))
                       output_port = "dataOut1(0)";

                    // AYA: 18/09/2023:
                    else if(nodes[i].type == "Un_Tagger" && indx == 0)
...
//                        }
                    ...
                    //}
                     // AYA: 07/08/2023: ignore the 2nd output of the LoopMux because I take care of it in the bottom
                    // Skip the Split component from the standard printing because it has its own custom output ports
                    if((nodes[i].component_operator.find("LoopMux") == std::string::npos && (nodes[i].component_operator.find("Split") == std::string::npos) ) || indx == 0) {
                    ...
                    if (nodes[i].component_operator.find("Split") != std::string::npos && indx == 1) {
                        // print the additional Split output
                        output_port = "dataOut2(0)";
            ...
            {            
                ...
                for ( int out_port_indx = 0; out_port_indx < components_type[nodes[i].component_type].out_ports; out_port_indx++) 
                {
                    ...
                    if(nodes[i].type == "Split")
                       output_port = "dataOut2(0)";

                    // AYA: 17/09/2023: added the following to print the extra tag port for the TAGGER node which is not labeled as tagged since its inputs are not tagged
                    else if(nodes[i].type == "Tagger" && tagger_is_tagging) {
                        // print the extra tag port
...
void vhdl_writer::write_vhdl ( string filename, int indx )
{
    ...
    bool tagged_loop_mux_flag = false; 
    bool tagger_is_tagging = true;  
